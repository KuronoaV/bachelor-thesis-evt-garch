# 03_eda.R
#
# Empirical verification of the stylised facts described in Section 4.3 of the
# thesis. The point of this script is not to discover anything new about
# equity-return tails — those facts are catalogued in Cont (2001) and reviewed
# at length in Chapter 2 — but to demonstrate that the two specific series
# used in this thesis exhibit them, with the same magnitudes and on the same
# axes that the reader will encounter again in Chapters 5 and 6.
#
# Three artifacts are produced. The full-sample summary table reports the
# number of observations, mean, standard deviation, sample skewness and
# Pearson kurtosis (m_4 / m_2^2, so the Normal benchmark is 3 not 0) of the
# daily log-loss for each index, computed on the entire post-1990 history.
# Sample skewness slightly above zero and kurtosis well above three confirm
# the leptokurtic, modestly
# right-skewed character of the loss distribution and motivate the EVT
# treatment of Chapters 3-6. The QQ plot against the Normal makes the same
# point graphically: both indices show pronounced upward curvature in the
# right tail, the regime that EVT models directly.
#
# The ACF plots address the second stylised fact — volatility clustering. The
# autocorrelation of the raw losses is small and largely contained within the
# Bartlett band, while the autocorrelation of the squared losses is uniformly
# positive and slowly decaying out to lag forty. This contrast is the
# empirical justification for the conditional-EVT step of Chapter 3.5: the
# i.i.d. assumption underlying unconditional EVT is violated by the data, and
# a GARCH filter is required before a GPD can legitimately be fitted to the
# residuals.
